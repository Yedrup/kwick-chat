(function() {
        ...
        //chat Controller
        function chatCtrl(kwickService, $rootScope, $timeout) {
            var chat = this;
            /* Scroll auto */
            chat.glued = true;
            ...
            //stop the timeout 
            if ($rootScope.isUserConnected = true) {
                $timeout(chat.updateChat, 2000);
            } else {
                chat.$on("$destroy", function() {
                        $timeout.cancel(chat.updateChat, 2000);
                    });
                }
            }
